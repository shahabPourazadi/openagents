...
export type PixelBlastProps = {
  ...
  edgeFade?: number;
  noiseAmount?: number;
  /** Optional image whose dark/ink pixels are folded into the dither feed. */
  logoSrc?: string;
  /** Logo size as a fraction of the viewport's shorter side (0–1). */
  logoScale?: number;
};
...
void main(){
  ...
  // Fold logo ink into the same Bayer feed so characters are made of blast pixels.
  if (uHasLogo > 0.5) {
    // Snap to the blast pixel grid (not the 8× noise cell) so glyphs stay sharp.
    vec2 quantCoord = (floor(gl_FragCoord.xy / pixelSize) + 0.5) * pixelSize;
    vec2 screenUV = quantCoord / uResolution;
    vec2 p = (screenUV - 0.5) * 2.0;
    float screenAspect = uResolution.x / uResolution.y;
    p.x *= screenAspect;

    float halfH = max(uLogoScale, 0.05);
    float halfW = halfH * max(uLogoAspect, 0.01);
    // Natural UV — left→right = O then A (Three.js flipY handles vertical).
    vec2 logoUV = vec2(
      p.x / (2.0 * halfW) + 0.5,
      p.y / (2.0 * halfH) + 0.5
    );
    ...
    if (logoUV.x >= 0.0 && logoUV.x <= 1.0 && logoUV.y >= 0.0 && logoUV.y <= 1.0) {
      vec4 logoSample = texture(uLogo, logoUV);
      float luma = dot(logoSample.rgb, vec3(0.299, 0.587, 0.114));
      // Terracotta/black → ink; cream/white eyes → holes
      float logoInk = smoothstep(0.90, 0.70, luma) * logoSample.a;
      // Additive boost so glyphs share dither/jitter with the ambient field.
      float breathe = 0.48 + 0.22 * fbm2(uv * 1.7, uTime * 0.12);
      feed += logoInk * breathe;
    }
  }
...
export function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const dummyLogo = new THREE.DataTexture(new Uint8Array([0, 0, 0, 0]), 1, 1);
      dummyLogo.needsUpdate = true;
      const uniforms = {
        ...
        uLogo: { value: dummyLogo as THREE.Texture },
        uHasLogo: { value: 0 },
      ...
      t.uniforms.uLogoScale.value = logoScale;
    ...
    if (t) {
      ...
      if (logoSrc) {
        ...
        loader.load(logoSrc, (tex) => {
          ...
          current.logoTexture?.dispose();
          current.logoTexture = tex;
          current.uniforms.uLogo.value = tex;
          const img = tex.image as { width?: number; height?: number } | undefined;
          current.uniforms.uLogoAspect.value =
            (img?.width || 1) / (img?.height || 1);
          current.uniforms.uHasLogo.value = 1;
        });
      } else {
        t.logoTexture?.dispose();
        t.logoTexture = undefined;
        if (t.dummyLogo) t.uniforms.uLogo.value = t.dummyLogo;
        t.uniforms.uHasLogo.value = 0;
      }
    }
    ...
    return () => {
      ...
      const cleanup = threeRef.current;
      cleanup.resizeObserver?.disconnect();
      cancelAnimationFrame(cleanup.raf!);
      cleanup.quad?.geometry.dispose();
      cleanup.material.dispose();
      cleanup.composer?.dispose();
      cleanup.logoTexture?.dispose();
      cleanup.dummyLogo?.dispose();
      cleanup.renderer.dispose();
      cleanup.renderer.forceContextLoss();
      if (cleanup.renderer.domElement.parentElement === container)
        container.removeChild(cleanup.renderer.domElement);
      threeRef.current = null;
    };
  }, [
    antialias,
